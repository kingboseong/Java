...
public class Test01_1 {
	public static void main(String[] args) {
		...
		arr[0] = s;
		arr[1] = i1;
//		int len = arr[0].length();//Object에 length()가 없다 / arr[0] = s; 는 실행했을 때의 값이 String 인것일 뿐.
									//arr[0]는 object이다 object에는 .length가 없어서 오류가 난다.
		int len = ((String) arr[0]).length(); // explicit type casting

		ArrayList lst = new ArrayList();//모든 타입의 객체들을 저장할 수 있다 그러나 비추다! - 이유는?
		String s2 = "java"; 
		Integer i2 = 123; //wrapper 클래스에 의해 박싱이 된 것.
		lst.add(s2); //스트링객체
		lst.add(i2); //정수객체
		...
		//원래 ArrayList는 전부 가능한데 <String>을 써줌으로서 Stirng만 쓸 수 있도록 바뀜.
...
//		list.to
		...
		int len_string = list.get(0).length(); //list는 string 객체이다 그 차이는(원인은?) 위에서 <String>을 써서?
		
		for (String element : list) { //배열이 아닌 ArrayList에도 확장형 for 사용 가능하다  *for 문은 눈 감고도 타이핑 가능 할 정도로 연습 (파이썬에서 매우 많이 사용함)
		...
		System.out.println("list.size() = " + list.size()); // size로 확인
//		list.size()
		...
		System.out.println("7 : " + list.contains("LA"));//boolean contains(Object o)를 사용한다  contain=>list에 LA가 있는지 없는지 확인
		
		// Object[] toArray() *** 중요
		Object obj[] = list.toArray();//toArray = list를 배열로 변경 - Object[] toArray()
		System.out.println("8 : " + obj);//obj는 참조 변수이고 이를 toString()으로 변환
		System.out.println("8 : " + Arrays.toString(obj));//배열의 원소들을 스트링으로 변화 / to는 변환이라고 생각하면 됨. toStirng은 Stirng으로 변환했다~는 위위에도 동일.
		...
		List<String> list2 = Arrays.asList("서울", "뉴욕", "상해");//asList()는 배열을 입력받아 리스트를 만든다. 입력받은 list를 tolist?로 줘라
		print(12, list2);
		...
		// boolean retainAll(Collection<?> c) => c=list / Collection이면 다 된다.??? 
