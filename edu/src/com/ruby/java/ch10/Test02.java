...
public class Test02 {
	...
	public static void main(String[] args) {
		...
		//page 460 그림
		Employee [] arr = new Employee[5]; // 객체배열
		arr[0] = new Employee();
		//
		arr[4] = new Employee();

		int [] num = new int[5]; //정수배열
		...
		ArrayList lst = new ArrayList(); //크기가 10인 리스트를 디폴트로 생성 => object임.
		lst.add(s);
		lst.add(ix);
		lst.add(1, "good");
		//		int len3 = lst.get(0).length();//오류 > list.get(0).은 Object이기 때문이다. 위에서 lst에 아무것도 할당하지 않았기 때문에object으로 선언돼있음.
		...
		list.add("도쿄");
		list.add("뉴욕");
		int len5 = list.get(0).length();//list.get(0).은 String을 반환한다 이유는 생성자에서 T(=<Stirng>)가 String으로 선언
		//E get(int index) >> generic으로 선언된 데이터 타입이므로 
		Collections.sort(list);
		//list.sort(comparator);
		System.out.println("0: 정렬후:: ");

		for (String element : list) { //( = int n : arr){  } 과 같음.
			System.out.print(element+" ");
		}
		System.out.println();

		for (int i = 0; i <list.size(); i++)
			System.out.print("list.size()를 사용한 출력:: " + list.get(i)); //i는 인덱스
		...
		//480페이지 1번째 문단 
		while (iter.hasNext()) { //(hasNext = T/F)
