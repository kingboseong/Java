...
class Tree5 {
	...
	void inorder() { // 이 두개는 다른 함수임. // driver 함수 // main 에서 부름
		inorder(root); // workhorse(일하는 말) = 계속 돌기만 함.
	}
	...
	void inorder(TreeNode5 CurrentNode) {
		if (CurrentNode != null) {
			inorder(CurrentNode.LeftChild); // 현재 데이터의 왼쪽을 모두 찍어라.
			System.out.print(" " + CurrentNode.data); // 현재 데이터 출력
			inorder(CurrentNode.RightChild); // 현재 데이터의 오른쪽을 모두 찍어라.
		}
	}
